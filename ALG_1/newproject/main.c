...
int main(){
    ...
    for(int t = 0; t < times; t++){
        ...
        int texts = 50000;
        FILE* daniel = fopen(daniel_file, "w+");
        ...
        for(int i = 0; i < texts; i++){
            ...
            for(int k = 0; k < 8; ++k){
                int unsigned_thing = plain[k];
                char* kay = int2bin(unsigned_thing);
                fprintf(daniel, "%s", kay);
                free(kay);
            }
            fprintf(daniel, " ");
            for(int k = 0; k < 8; ++k){
                int unsigned_thing = cipher[k];
                char* kay = int2bin(unsigned_thing);
                fprintf(daniel, "%s", kay);
                free(kay);
            }
    ...
    printf("%lf",sum_times/times);
}


char* int2bin(int num){
    char* daniel = malloc(8);
    for(int i = 7; i >= 0; --i){
        daniel[i] = num%2 ? '1':'0';
        num = num/2;
    }
