...
vector<vector<vector<vector<double>>>> pre_calculated_mat;

vector<int> plain_L_mask{8, 16, 22, 30};
vector<int> plain_R_mask{7, 13, 24, 2, 31, 0, 1, 2, 3, 4};
vector<int> cipher_L_mask{7, 13, 24, 2};
vector<int> cipher_R_mask{8, 16, 22, 30, 15, 16, 17, 18, 19, 20};

// key_mask[i] - the key bit we use in level i+1
vector<int> key_mask{27, 51, 3, 48, 38, 16, 6, 49, 45, 25, 13, 58, 44, 26, 12, 2};  // starts at 0

vector<int> k1_s1_mask{10, 51, 34, 60, 49, 17};
vector<int> k16_s5_mask{30, 5, 47, 62, 45, 12};
vector<int> first_last_mask = {10, 51, 34, 60, 49, 17, 30, 5, 47, 62, 45, 12};
vector<int> key_mask_14_mid_rounds{51, 3, 48, 38, 16, 6, 49, 45, 25, 13, 58, 44, 26,
...
string getSubInput(string input, const vector<int> &mask) {
    string sub_str;
    for (int i : mask) {
        sub_str += input[i];
    }
...
int calcIndex(int num_of_rounds, string binary_used_key) {
    pair<string, string> data = eliaFunc(binary_used_key, num_of_rounds);
    string bin_plain_r = data.first.substr(0, 32);
    string bin_plain_l = data.first.substr(32, 32);
    string bin_cipher_r = data.second.substr(0, 32);
    string bin_cipher_l = data.second.substr(32, 32);

    string sub_plain_r = getSubInput(bin_plain_r, plain_R_mask);
    string sub_plain_l = getSubInput(bin_plain_l, plain_L_mask);
    string sub_cipher_r = getSubInput(bin_cipher_r, cipher_R_mask);
    string sub_cipher_l = getSubInput(bin_cipher_l, cipher_L_mask);

    string sub_plain = sub_plain_l + sub_plain_r;
    string sub_cipher = sub_cipher_l + sub_cipher_r;
    string sub_input = sub_plain + sub_cipher;
...
string intToBinStr(int num, int len) {
    string bin_str;
    int new_num = num;
    while (new_num > 0) {
        int res = new_num % 2;
...
pair<int, int> calculate_P_C_from_key_combination(int key, int combination) {
    string binary_key = intToBinStr(key, 12);
    string k1 = binary_key.substr(0, 6);
    string k16 = binary_key.substr(6, 6);

    // combination - p out s1 (4)|| p out s5 (4) || p in s1 (6)|| c out s5 (4)|| c out s1 (4)|| c in s5 (6)
    // combination -     0-3     ||     4-7      ||    8-13    ||    14-17    ||    18-21    ||   22-27
    string binary_combination = intToBinStr(combination, 28);

    // plaintext - (out S5)_2 || (out S1)_2 = (out S5)_1 || ((out S1)_1 xor S1(in S1, K1))
    string plain_left = binary_combination.substr(4, 4);  // (out S5)_1
    string binary_input1 = binary_combination.substr(8, 6);  // in S1
    string input_to_s1 = intToBinStr(binaryStrToInt(binary_input1) ^ binaryStrToInt(k1), 6);
    string out_s1_1 = binary_combination.substr(0, 4);
    string plain_right = intToBinStr(binaryStrToInt(out_s1_1) ^ sboxFunction(1, input_to_s1), 4);
    int plaintext = binaryStrToInt(plain_left + plain_right);

    // ciphertext - (out S1)_2 || (out S5)_2 = (out S1)_1 || ((out S1)_1 xor S5(in S5, K16))
    string cipher_left = binary_combination.substr(18, 4);
    string binary_input16 = binary_combination.substr(22, 6);
    string input_to_s5 = intToBinStr(binaryStrToInt(binary_input16) ^ binaryStrToInt(k16), 6);
    string out_s5_1 = binary_combination.substr(14, 4);
    string cipher_right = intToBinStr(binaryStrToInt(out_s5_1) ^ sboxFunction(5, input_to_s5), 4);
...
int attackAlgorithm2(int num_of_rounds, int num_of_inputs, string &binary_used_key) {
     ...
     */
    vector<vector<vector<double>>> input_matrix{static_cast<size_t>(pow(2, 12)),
                                                vector<vector<double>>{matrix_size, vector<double>(matrix_size, 0)}};

    for (int key = 0; key < pow(2, 12); key++) {
        for (int combination = 0; combination < pow(2, 28); combination++) {
            pair<int, int> plain_cipher_pair = calculate_P_C_from_key_combination(key, combination);
    ...
    string str_curr_first_last_key = getSubInput(binary_used_key, first_last_mask);
    string str_curr_middle_key = getSubInput(binary_used_key, key_mask_14_mid_rounds);
    string str_curr_middle_key_by_rounds = str_curr_middle_key.substr(0, num_of_rounds);
