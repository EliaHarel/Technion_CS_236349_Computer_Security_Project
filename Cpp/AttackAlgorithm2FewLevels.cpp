...
vi s1_out_mask{8, 16, 22, 30};
vi s5_out_mask{7, 13, 24, 2};
...
int calcOutSbox (long long text, vi& mask){
    vi shift;
    for(int i=0; i<mask.size(); i++){
        shift.emplace_back(31 - mask[i]); // 32 is the length in base 2 of text, mask[i] in {0,...,31}
    }
    int c1= ((text & (1<<shift[0]))>> shift[0])<<3;
    int c2= ((text & (1<<shift[1]))>>shift[1])<<2;
    int c3= ((text & (1<<shift[2]))>> shift[2])<<1;
    int c4= (text & (1<<shift[3]))>>shift[3];
...
int attackAlgorithm2FewLevels(int num_of_rounds, int num_of_inputs, vvvvd& pre_calculated_mat){
    ...
    for(int i = 0; i < num_of_inputs; i ++){
        ...
        long long p_left = binaryStrToInt(plain_left);
        long long p_right = binaryStrToInt(plain_right);
        long long c_left = binaryStrToInt(cipher_left);
        long long c_right = binaryStrToInt(cipher_right);


        int s1_in = ((p_right& 1)<<5) & (p_right>>27); // bits 32,1,2,3,4,5 of p_right;
        int s5_in = (c_right>>11) & 63 ; // bits 16,17,18,19,20, 21 of c_right;

        int s1_out_first = calcOutSbox(p_left, s1_out_mask);
        int s5_out_first =calcOutSbox(p_right, s5_out_mask); // char_plain_left
        int s1_out_last = calcOutSbox(c_right, s1_out_mask); // char_chiper_left
        int s5_out_last =calcOutSbox(c_left, s5_out_mask);
        ...
        for(int first_last_key = 0; first_last_key < NUM_OF_FIRST_LAST_KEYS; first_last_key ++){
            ...
            int char_cipher_right = s5_out_last ^ sboxFunction(5, input_s5);

            int char_plain = (s5_out_first << 4) + char_plain_right;
            int char_cipher = (s1_out_last << 4) + char_cipher_right;
