...
#include "ShortTablesAttackAlgorithm2FewLevels.h"
...
void attack(int attackNumber, int rounds, int plain_cipher_pairs, int iterations,
    ...
    for(int i = 1; i <= iterations; ++i){
        int location;
        location = ShortTablesattackAlgorithm2FewLevels(rounds, plain_cipher_pairs, pre_calculated_mat);
    ...
    output_file << std::endl << "Average - " + std::to_string(rounds) + " rounds, " +
                                std::to_string(iterations) + " iterations, " +
                                std::to_string(plain_cipher_pairs) + " pairs - " +
                                std::to_string(avg_location) << std::endl;
