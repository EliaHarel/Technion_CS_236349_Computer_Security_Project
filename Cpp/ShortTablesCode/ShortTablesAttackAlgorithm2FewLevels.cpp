...
int ShortTablesattackAlgorithm2FewLevels(int num_of_rounds, int num_of_inputs, vvvvd& pre_calculated_mat){
    ...
    for(int first_last_key = 0; first_last_key < NUM_OF_FIRST_LAST_KEYS; first_last_key++){
        for(int middle_key = 0; middle_key < pow(2, ROUNDS_OF_SHORTEN_TABLE); middle_key++){
            if(first_last_key == curr_first_last_key && middle_key == curr_middle_key_by_rounds) continue;
//TODO: inform Stav
/*  in the shortened method we cannot check the "middle key" because now we are using a
    representation of the key and not the key itself */
