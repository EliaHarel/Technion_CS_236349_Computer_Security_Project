...
void Calc16Tables(int rounds, vvvvd& pre_calculated_mat){
    ...
    for(int i = ready_rounds + 2; i <= wanted_rounds; i += 2){
    ...
    ready_rounds = wanted_rounds;

    for(int i = ready_rounds + 2; i <= rounds; i += 2){
        for(int j = 0; j < pow(2, ROUNDS_OF_SHORTEN_TABLE); j++){
            matrixMul(pre_calculated_mat.at(i - 2).at(j), pre_calculated_mat.at(2).at(0),
                      // using 0, because we are only multipling by the [0,0] part of the second level
                      pre_calculated_mat.at(i).at(j));
        }
    }
}
