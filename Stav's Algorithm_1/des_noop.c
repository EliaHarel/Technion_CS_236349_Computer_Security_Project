...
void DES_decrypt(char cipher[8], char key[8], char plain[8])
{
	...
	for(i=0; i<8; i+=2) {
	  t[0] ^= f(t[1], processed_key[7-i]);
	  t[1] ^= f(t[0], processed_key[7-i-1]);
	}
...
void expand(inp, out)        /* E inplementation */
...
void process_key(char key[8], char processed_key[16][8])
{
  int i;
